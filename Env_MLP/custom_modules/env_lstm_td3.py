...
class CarEnv(gym.Env):
    ...
    def __init__(self, seed=0, port=2000):
        ...
        self._stop_gate = 0.0
        self._last_norm_signed_dev = 0.0
        self._last_norm_heading_err = 0.0
        self.image_for_CNN = None
    ...
    def step(self, action):
        ...
        if kmh < 1.0 and not red_light and self._stop_gate < 0.5:
        ...
        # L1 lateral offset reference
        right = waypoint.transform.get_right_vector()
        ref_location = carla.Location(
            x = waypoint.transform.location.x + self._lateral_offset * right.x,
            y = waypoint.transform.location.y + self._lateral_offset * right.y,
            z = waypoint.transform.location.z
            )
        # 
        signed_dev_mag = ref_location.distance(loc_after)
        dx = loc_after.x - ref_location.x
        dy = loc_after.y - ref_location.y
        cross_z = (w_fwd.x*dy) - (w_fwd.y*dx)
        signed_c_dev = math.copysign(signed_dev_mag, cross_z) if cross_z != 0.0 else signed_dev_mag
        norm_signed_dev = float(np.clip(signed_c_dev / 3.0, -1.0, 1.0))

        self._last_norm_signed_dev = norm_signed_dev
        self._last_norm_heading_err = norm_heading_err
        ...
        red_light_val = 1.0 if (red_light or self._stop_gate >= 0.5) else 0.0
        ...
        info = {"Kmph": float(kmh), "wp_dev":float(wp_dev), 
                "signed_c_dev": float(signed_c_dev),
    ...
    def set_plan(self, lateral_offset, stop_gate):
        "L1 sub-goal planner, L2 obs channels."
        self._lateral_offset = float(np.clip(lateral_offset, -1.0, 1.0))
        self._stop_gate = 1.0 if float(stop_gate) >= 0.5 else 0.0

    def _next_tl_info(self, max_d=40.0):

        "Nearest TL ahead of ego"
        ...
        for tl in self.world.get_actors().filter('*traffic_light*'):
            ...
            for wp in stops:
                ...
                if (to.x*fwd.x + to.y*fwd.y) <= 0.0:
                    continue
                d = wp.transform.location.distance(loc)
                if d > max_d:
                    continue
                if best_d is None or d < best_d:
                    best_d, best_state = d, tl.get_state()
        if best_d is None:
            return 1.0, [0.0, 0.0, 0.0]
        return float(best_d/max_d), [
            1.0 if best_state == carla.TrafficLightState.Red else 0.0,
            1.0 if best_state == carla.TrafficLightState.Yellow else 0.0,
            1.0 if best_state == carla.TrafficLightState.Green else 0.0 ]

    def get_L1_features(self):
        v = self.vehicle.get_velocity()
        norm_speed = float(np.clip(3.6*math.sqrt(v.x**2 + v.y**2 + v.z**2)/90.0, 0, 1))
        dist_tl, onehot = self._next_tl_info()

    # wp curvature
        cur = self.map.get_waypoint(self.vehicle.get_location(), project_to_road = True, 
                                    lane_type = carla.LaneType.Driving)
        prev_yaw, curv = cur.transform.rotation.yaw, []

        for _ in range(4):
            nx = cur.next(2.0)
            if not nx: curv.append(0.0); continue
            cur = nx[0]
            dyaw = (cur.transform.rotation.yaw - prev_yaw + 180) % 360 -180
            curv.append(float(np.clip(dyaw/45.0, -1, 1)))
            prev_yaw = cur.transform.rotation.yaw
        return np.array([norm_speed, self._last_norm_signed_dev, self._last_norm_heading_err,
                         dist_tl, *onehot, *curv], dtype=np.float32)
